...
def is_valid_email(email):
    # Very basic email validation, you can use a more sophisticated method
    return "@" in email and ".com" in email and email.endswith("@gmail.com")
...
def SubmitData():
    if (
        ...
        result = tkMessageBox.showwarning(
            "", "Invalid Email Address (should be a Gmail address)", icon="warning"
        )
    else:
        tree.delete(*tree.get_children())
        conn = sqlite3.connect("contact.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO `member` (firstname, email, age, contact) VALUES(?, ?, ?, ?)", (str(FIRSTNAME.get()), str(EMAIL.get()), str(AGE.get()), str(CONTACT.get())))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `firstname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def UpdateData():
    if not is_valid_phone_number(CONTACT.get()):
        ...
        conn = sqlite3.connect("contact.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `email` = ?, `age` = ?, `contact` = ? WHERE `mem_id` = ?", (str(FIRSTNAME.get()), str(EMAIL.get()), str(AGE.get()), str(CONTACT.get()), int(mem_id)))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `firstname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
